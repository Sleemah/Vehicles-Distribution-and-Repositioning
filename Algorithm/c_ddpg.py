...
class CNet(BasicModule):   # ae(s)=a
    def  __init__(self,s_dim,a_dim,name):
        ...
        self.fc1 = nn.Linear(s_dim+a_dim+32*3*1,128)
        nn.init.xavier_normal_(self.fc1.weight)
        nn.init.constant_(self.fc1.bias, 0)
        self.bn1 = nn.BatchNorm1d(128)
        #self.fc1.weight.data.normal_(0,0.1)   # initialization
        ...
        #self.out.weight.data.normal_(0, 0.1)  # initialization
    def forward(self,s,a,map_ds):
        s,a,map_ds = s.to(device),a.to(device),map_ds.to(device)
...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound):
        ...
        self.memory = np.zeros((MEMORY_CAPACITY, s_dim * 2 + a_dim +self.mapSize*2+self.mapSize*2+ 1), dtype=np.float32)
        self.pointer = 0
    ...
    def learn(self):
        ...
        bmap = torch.FloatTensor(np.reshape(bt[:, self.s_dim + self.a_dim: self.s_dim + self.a_dim+self.mapSize*2],(-1,2,self.mapWidth,self.mapHeight))).to(device)
        bmap_ = torch.FloatTensor(np.reshape(bt[:, -self.s_dim - 1 - self.mapSize*2:-self.s_dim - 1],(-1,2,self.mapWidth,self.mapHeight))).to(device)
        br = torch.FloatTensor(bt[:, -self.s_dim - 1: -self.s_dim]).to(device)
        bs_ = torch.FloatTensor(bt[:, -self.s_dim:]).to(device)

        a = self.Actor_eval(bs)
        q = self.Critic_eval(bs,a,bmap)  # loss=-q=-ce（s,ae（s））update ae   ae（s）=a   ae（s_）=a_
       # If a is a correct behavior, then its Q should be closer to 0
        ...
        a_ = self.Actor_target(bs_)  # This network does not update parameters in time to predict actions in Critic's Q_target
        q_ = self.Critic_target(bs_,a_,bmap_)  # This network does not update the parameters in time, which is used to give the Gradient ascent strength when the Actor updates the parameters
        q_target = br+GAMMA*q_  # q_target = minus
        q_v = self.Critic_eval(bs,ba,bmap)
        #print(q_v)
    ...
    def store_transition(self, s, a, map, map_,r, s_):
        transition = np.hstack((s, a, np.reshape(map,(-1)),np.reshape(map_,(-1)) ,[r], s_))
